 ...
 * @author esh
 */
public class FrameEmployeeSummaryReport extends javax.swing.JFrame {
     ...
     */
    public FrameEmployeeSummaryReport() {
        initComponents();
    }

    public FrameEmployeeSummaryReport(String LOCATIONID, String USERNAME) {
        initComponents();
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "USER", "COUNT", "COST", "AMOUNT", "DISCOUNT", "NETAMOUNT", "PROFIT"
            }
        ) {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (jDateChooserFrom.getDate() == null || jDateChooserTo.getDate() == null) {
            ...
            try {
                ...
                ResultSet rs = cls.db.myConnection().createStatement().executeQuery("SELECT DISTINCT idemployeeMaster FROM invoice WHERE  date BETWEEN  '" + dd + "' AND '" + dd1 + "'");

                while (rs.next()) {
                    Vector v = new Vector();

                    String idemployee = rs.getString("idemployeeMaster");
                    
                    ResultSet rs1 = cls.db.myConnection().createStatement().executeQuery("SELECT * FROM user WHERE  iduser='" +idemployee+ "' ");

                    if (rs1.next()) {
                        v.add(rs1.getString("userName"));

                    }

                    ResultSet rs2 = cls.db.myConnection().createStatement().executeQuery("SELECT SUM(qty) FROM invoice WHERE  idemployeeMaster='" + idemployee + "' ");

                    if (rs2.next()) {
                        v.add(rs2.getDouble("SUM(qty)"));
                    }

                    ResultSet rs3 = cls.db.myConnection().createStatement().executeQuery("SELECT SUM(cost) FROM invoice WHERE  idemployeeMaster='" + idemployee + "' ");

                    if (rs3.next()) {
                        v.add(rs3.getString("SUM(cost)"));
                        tot_cost=rs3.getDouble("SUM(cost)");
                    }
                    
                    ResultSet rs4 = cls.db.myConnection().createStatement().executeQuery("SELECT SUM(amount) FROM invoice WHERE  idemployeeMaster='" + idemployee + "' ");

                    if (rs4.next()) {
                        v.add(rs4.getDouble("SUM(amount)"));
                    }
                    
                    ResultSet rs5 = cls.db.myConnection().createStatement().executeQuery("SELECT SUM(discount) FROM invoice WHERE  idemployeeMaster='" + idemployee + "' ");

                    if (rs5.next()) {
                        v.add(rs5.getDouble("SUM(discount)"));
                    }
                    
                    ResultSet rs6 = cls.db.myConnection().createStatement().executeQuery("SELECT SUM(net_amt) FROM invoice WHERE  idemployeeMaster='" + idemployee + "' ");
...
//                    v.add(rs.getString("date"));
                ...
                for (int i = 0; i < jTable1.getRowCount(); i++) {
                        ...
                        String net_amt = jTable1.getValueAt(i, 5).toString();
                        String profit = jTable1.getValueAt(i, 6).toString();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FrameEmployeeSummaryReport.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new FrameEmployeeSummaryReport().setVisible(true);
            }
        });
    }
